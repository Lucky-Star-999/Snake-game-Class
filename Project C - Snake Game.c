	...
	if (kbhit())
        {
            ...
            //Key = 13 means the user have pressed the ENTER key
            if (key == 13)
            	goto AGAIN;
            	
            //Key = 27 means the user have pressed the ESC key
            else if (key == 27)
            	goto END;
        }
    }
...
void control()
{
	...
	*/
	...
	if (kbhit())
        {
			//tt: the enum-variable to tell the current state of the game, it just receives only 4 values (UP, DOWN, LEFT, RIGHT).
			//The enum variable make easier for everyone to read.
			//If tt = LEFT, it means that the current state of the snake's head is LEFT, and the snake is moving leftwards.
...
void body(int *ptrx, int *ptry, int currentscore)
{
	...
	//ptrx[]: array containing the position of all parts of snake, in X-axis
	//ptry[]: array containing the position of all parts of snake, in Y-axis
	//Delete the snake's tail
	gotoxy(ptrx[currentscore + 1] , ptry[currentscore + 1]);
	printf(" ");
	
	//Copy the position of the part of body to the next part
...
void touchbody(int *ptrx, int *ptry, int *count, int currentscore)
{
	...
	//Checking if the snake's head touches its body or not
	//ptrx[]: array containing the position of all parts of snake, in X-axis
	//ptry[]: array containing the position of all parts of snake, in Y-axis
	for(i=2; i<=currentscore; i++){
		if (ptrx[0] == ptrx[i] && ptry[0] == ptry[i]){
			...
			//The "countover" will not be zero, so it will tell the program that it will be in "Game over" state
			(*count)++;
		}
	}
	...
	if (kbhit())
        {
			...
			//key = 13 means the user have pressed the ENTER key
			//At the position y = 13, the triangle is pointing the line "Start game"
			if (key == 13 && y==13)
				return 1;
				
			//If the triangle points to the line "Exit", then exit the game
			//key = 13 means the user have pressed the ENTER key
			//At the position y = 15, the triangle is pointing the line "Exit"
			if (key == 13 && y==15)
				return 2;	   
	}
}
